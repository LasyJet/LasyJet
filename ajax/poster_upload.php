<?php
    ...
    if ( 0 < $_FILES['file']['error'] ) {
        ...
        if (in_array($ext, $allowed)) {
            ...
            $path="/uploads/poster/";
            $filename=transliterator_transliterate('Any-Latin; Latin-ASCII; Lower()', $fullname).date("Ymd_H-i").".".$ext;
            $filename=str_replace([" ","*"],["_",""],$filename);
            $filename=$poster_num."=".$filename;
            if(!move_uploaded_file($_FILES['file']['tmp_name'], "../".$path.$filename))
                  $err=" ERROR";
                        #========================
                        // *** INSERT POSTER NUMBER ***
            ...
            if(!empty($_SESSION['user_id'])){
                ...
                $data=["thesis_id"=>$_SESSION['thesis_id'], "poster_file"=>$path.$filename];
                $sql="UPDATE ".YEAR."_thesises SET `poster_file`=:poster_file WHERE `thesis_id`=:thesis_id";
                $stmt = $dbh->prepare($sql);
                $stmt->execute($data);
            }
            
            $file=SITE.$path.$filename;
            $status="Your file <a href='".$file."'>successfully uploaded</a>";
